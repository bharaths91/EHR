...
def get_keys():
    ...
    print(public_key)
    print(private_key)
    pub_key = public_key.save_pkcs1() ###stave it in PKSC
    pri_key = private_key.save_pkcs1() ###stave it in PKSC
...
def rsa_encryption(in_file, out_file, public_key, key_length=32):
    ...
    password = str(random.randint(1000, 10000)) ###Aes key symmetric key
    key, iv = derive_key_and_iv(password, salt, key_length, bs)###Aes key symmetric key with salt initilizationvector.
    cipher = AES.new(key, AES.MODE_CBC, iv)
    out_file.write(salt)
    finished = False

    while not finished:
        chunk = in_file.read(1024 * bs) ##take the infile 1026 * 16 bytes.
        if len(chunk) == 0 or len(chunk) % bs != 0:  # final block/chunk is padded before encryption
            padding_length = (bs - len(chunk) % bs) or bs
            chunk += str.encode(padding_length * chr(padding_length))
            finished = True
        out_file.write(cipher.encrypt(chunk)) ###printing the file until encrypted. 
    new_public_key = rsa.PublicKey.load_pkcs1(public_key.encode())
    enc_text = rsa.encrypt(password.encode(), new_public_key)###encrypt the password with public key.
    print(password)
    return enc_text
...
def rsa_decryption(in_file, out_file, enc_text, private_key, key_length=32):
    ...
    dec_text = rsa.decrypt(enc_text, new_private_key)###get the common key.
